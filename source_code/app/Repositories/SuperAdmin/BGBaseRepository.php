...
abstract class BGBaseRepository extends BaseRepository
{
    ...
    public function findByField($field, $condition = '=', $value = null, $columns = ['*'], $is_paging = true, $limit = 16)
    {
        $this->applyCriteria();
        $this->applyScope();
        $limit = is_null($limit) ? config('repository.pagination.limit', 16) : $limit;
    ...
    public function findWhere(array $where, $columns = ['*'], $is_paging = true, $limit = 16)
    {
        $this->applyCriteria();
        $this->applyScope();
    ...
    public function findWhereIn($field, array $values, $columns = ['*'], $is_paging = true, $limit = 16)
    {
        $this->applyCriteria();
        $this->applyScope();
        $limit = is_null($limit) ? config('repository.pagination.limit', 16) : $limit;
    ...
    public function findWhereNotIn($field, array $values, $columns = ['*'], $is_paging = true, $limit = 16)
    {
        $this->applyCriteria();
        $this->applyScope();
    ...
    public function getAllForSelectBoxFile($columns = ['*'], $id = null, $blank = true, $defaultValue = "Chọn giá trị")
    {
        $categoryList = $this->model->all($columns);
        $categories = array();
        if ($blank) $categories[0] = '-- ' . $defaultValue . ' --';
        foreach ($categoryList as $category) {
            if ($category->id != $id || $id == null) {
                $categories[$category->id] = $category->content;
            }
        }
