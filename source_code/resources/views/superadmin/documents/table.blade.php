    ...
    @else
        @foreach($documents as $key=>$document)
            <tr>
                ...
                <td width="40px"><input type="checkbox" name="ids[]" value="{{ $document->id }}"
                                        class="minimal checkSingle"
                                        form="items"/></td>
                <td>
                    <a href="{!! route('superadmin.files.index') !!}?search[document_id]={{ $document->id }}">{{$document->name}}</a>
                </td>
                ...
                <td style="min-width: 110px">
                    ...
                    <div class='btn-group'>
                        <a href="{!! route('superadmin.documents.show', [$document->id]) !!}"
                           class='btn btn-default btn-xs'><i
                                    class="glyphicon glyphicon-eye-open"></i></a>
                        @if(Auth::user()->id == $document->user->id)
                            <a href="{!! route('superadmin.documents.edit', [$document->id]) !!}"
                               class='btn btn-default btn-xs'><i
